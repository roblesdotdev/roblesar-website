...
import { type NoteListing } from '~/utils/notes.server'

export function NoteItem({ note }: { note: NoteListing }) {
  return (
    <li key={note.slug} className="flex flex-col gap-1">
      ...
      <Link to={`/notes/${note.slug}`} prefetch="intent" className="self-start">
        <h1>{note.title}</h1>
      </Link>
